...
# Use GNU Make $(file ...) so Windows NDK does not run printf via a broken shell (corrupts jconfig.h).
jconfig_sharp := \#
$(file > $(JPEG_LIB_PATH)/jconfig.h,/* autogenerated jconfig.h based on Android.mk var JCONFIG_FLAGS */)
$(foreach name,$(JCONFIG_FLAGS), \
    $(if $(findstring =,$(name)), \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)ifndef $(firstword $(subst =, ,$(name)))) \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)define $(subst =, ,$(name))) \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)endif) \
    , \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)ifndef $(name)) \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)define $(name)) \
        $(file >> $(JPEG_LIB_PATH)/jconfig.h,$(jconfig_sharp)endif) \
    ) \
)

# create definition file jconfigint.h, needed in order to build
$(file > $(JPEG_LIB_PATH)/jconfigint.h,/* autogenerated jconfigint.h based on Android.mk vars JCONFIGINT_FLAGS */)
$(foreach name,$(JCONFIGINT_FLAGS),$(file >> $(JPEG_LIB_PATH)/jconfigint.h,$(jconfig_sharp)define $(subst =, ,$(name))))
