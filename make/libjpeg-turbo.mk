...
# (POSIX sh: avoid `echo /*` which glob-expands; avoid `echo #...` where `#` starts a comment)
$(shell printf '%s\n' '/* autogenerated jconfig.h based on Android.mk var JCONFIG_FLAGS */' > $(JPEG_LIB_PATH)/jconfig.h)
$(foreach name,$(JCONFIG_FLAGS), \
    $(if $(findstring =,$(name)), \
        $(shell printf '%s\n' \
            '#ifndef $(firstword $(subst =, ,$(name)))' \
            '#define $(subst =, ,$(name))' \
            '#endif' \
...
$(shell printf '%s\n' '/* autogenerated jconfigint.h based on Android.mk vars JCONFIGINT_FLAGS */' > $(JPEG_LIB_PATH)/jconfigint.h)
$(foreach name,$(JCONFIGINT_FLAGS),$(shell printf '%s\n' '#define $(subst =, ,$(name))' >> $(JPEG_LIB_PATH)/jconfigint.h))
